# `crkbd_qmk`
> My custom keymap for QMK [foostan Corne V4.1's](https://github.com/foostan/crkbd/).

![photo of keyboard](alt/image1.jpg)
*More images in the [`alt/` folder](https://github.com/x3hy/crkbd_qmk/tree/master/alt) along with other misc layout files.*

## Layout:
![layout image](alt/layout.png)
> 
## Combos:
Combos are annoying on the keymap drawer so here is a list of combos from the [`x3hy/keymap.c` file](https://github.com/x3hy/crkbd_qmk/tree/master/x3hy/keymap.c), generated using *`make combos`*
...
## Flashing
QMK does not work correctly with my system, specifically QMK flashing. To remedy this I've written a custom script ([`src/flash`](https://github.com/x3hy/crkbd_qmk/tree/master/src/flash)) that correctly flashes the new firmware to the board.
<br></br>
To flash the firmware just run the following:
```
make flash
```
**You will be prompted for sudo** so please ensure you have read the [`Makefile`](https://github.com/x3hy/crkbd_qmk/tree/master/Makefile) and the [`src/flash`](https://github.com/x3hy/crkbd_qmk/tree/master/src/flash) file.

## Thoughts
> This is filled with bias, its just my opinion, do not take it unbiased insight.
